...
from info_models import SquadDIMLoss
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        sequence_output, _, embedding_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            ### compute QAInfomax regularizer loss ###
            context_enc = [] # context means {question, paragraph}, which is different to the paper in naming.
            ans_enc = [] 
            batch_size = sequence_output.size(0)
            for b_idx in range(batch_size):
                if start_positions[b_idx].item() == 0 and end_positions[b_idx].item() == 0: # invalid example or impossible example
                    continue

                context_output = torch.cat((sequence_output[b_idx, :start_positions[b_idx], :],
                                            sequence_output[b_idx, end_positions[b_idx] + 1:, :]),
                                            dim=0)
