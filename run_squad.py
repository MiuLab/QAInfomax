...
def main():
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    writer.add_scalar('Info_Loss', info_loss.item(), global_step)
                    global_step += 1
    ...
    if args.do_train:
        ...
        state.update(torch.load(output_model_file))
        state = {k: state[k] for k in model.state_dict()}
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                batch_start_logits, batch_end_logits, batch_features = model(input_ids, segment_ids, input_mask)
            for i, example_index in enumerate(example_indices):
                start_logits = batch_start_logits[i].detach().cpu().tolist()
                end_logits = batch_end_logits[i].detach().cpu().tolist()
                features = batch_features[i].detach().cpu() # (sequence , hidden)
